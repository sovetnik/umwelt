defmodule Umwelt.Parser.Attrs do
  ...
  defp parse_children({term, _, _}, _context)
       when term in ~w|behaviour opaque typep|a,
       do: nil

  defp parse_children({term, _, value}, _context)
       when term in ~w|doc moduledoc|a,
  ...
  defp parse_children({:typedoc, _, value}, context),
    do: %{typedoc: Parser.maybe_list_parse(value, [], context)}

  defp parse_children({:impl, _, value}, context),
    do: %{impl: Parser.maybe_list_parse(value, [], context)}

  defp parse_children({constant, _, [child]}, context),
    do: %Attribute{
      name: to_string(constant),
      value: Parser.parse(child, [], context)
    }
